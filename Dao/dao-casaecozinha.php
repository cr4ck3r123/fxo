...
function listarCasaCozinha($pagina, $itens_por_pagina) {
    $pdo = conecta();
    try {
        $listar = $pdo->query("SELECT * FROM produtos where categorias_id = 4 LIMIT $pagina, $itens_por_pagina");
        $listar->execute();
...
function listarCasaCozinhaTotal() {
    $pdo = conecta();
    try {
        $listar = $pdo->query("SELECT * FROM produtos where categorias_id = 4");
        $listar->execute();
...
function recuperaDadosCasa($id) {
    $pdo = conecta();
    try {
        $pegaDados = $pdo->prepare("SELECT * FROM produtos WHERE idprodutos = ?");
