...
function listarInformatica($pagina, $itens_por_pagina) {
    $pdo = conecta();
    try {
        $listar = $pdo->query("SELECT * FROM produtos where categorias_id = 3 LIMIT $pagina, $itens_por_pagina");
        $listar->execute();
...
function listarInformaticaTotal() {
    $pdo = conecta();
    try {
        $listar = $pdo->query("SELECT * FROM produtos where categorias_id = 3");
        $listar->execute();
...
function recuperaDadosInformatica($id) {
    $pdo = conecta();
    try {
        $pegaDados = $pdo->prepare("SELECT * FROM produtos WHERE id = ?");
