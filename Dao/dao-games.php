...
//FUNÇÃO LISTAR
function listarSmartphones($pagina, $itens_por_pagina) {
    $pdo = conecta();
    try {
        $listar = $pdo->query("SELECT * FROM produtos where categoria = 5 LIMIT $pagina, $itens_por_pagina");
        $listar->execute();
...
//FUNÇÃO LISTAR
function listarSmartphonesTotal() {
    $pdo = conecta();
    try {
        $listar = $pdo->query("SELECT * FROM produtos where categoria = 5;");
        $listar->execute();
...
function VerificaEstoque($id) {
    $pdo = conecta();
    try {
        $qtde = $listar = $pdo->query("SELECT qtde FROM eletronicos where id = ?");
