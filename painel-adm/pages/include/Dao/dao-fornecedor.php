...
//FUNÇÃO LISTAR
function listarFornecedor($pagina, $itens_por_pagina) {
    $pdo = conecta();
    try {
        $listar = $pdo->query("SELECT * FROM fornecedores LIMIT $pagina, $itens_por_pagina;");
        $listar->execute();
...
function listarTotal() {
    $pdo = conecta();
    try {
        $listar = $pdo->query("select * from fornecedores;");
        $listar->execute();
